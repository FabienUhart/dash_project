...
4. [ ] 2e passe : évaluer la mutualisation de la **logique de rendu carte** (`draw`, `renderSubbar`, `renderGroupbar`, `memoRow`, focus helpers) — corps déjà très proches, à factoriser en passant `state`/`DATA` en paramètre.
...
6. [ ] (Ultérieur, à arbitrer) paramétrer `renderSidebar`/`renderTree` via une couche d'accès aux données commune pour retirer la dernière grosse duplication.
